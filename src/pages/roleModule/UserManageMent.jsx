...
export default function UserManageMent() {
  ...
  const saveDoctor = async () => {
    if (!doctorName.trim() || !doctorQualification.trim()) return;
  ...
  const deleteDoctor = async (id) => {
    ...
    try {
      await ApiDelete(`/admin/doctor/${id}`);
      fetchDoctors();
    } catch (err) {
  ...
  const addDoctor = () => {
    ...
    setDoctorList([
      ...doctorList,
      { id: Date.now(), name: doctorName, qualification: doctorQualification },
    ]);
    setDoctorName("");
    setDoctorQualification("");
    setIsDoctorModalOpen(false);
  ...
  //   setDoctorQualification(doctor.qualification);
  //   setIsDoctorModalOpen(true);
  //   deleteDoctor(id); // remove and re-add on save
  // };
